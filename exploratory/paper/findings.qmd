...
- **MapSwipe is never a headline metric.** Its ~50 m cell verdicts, seeded from AI flags, make
  ...
  building-level reference, and not usable to score the day-zero baseline (would be circular,
...
day-zero baseline at area level (it plausibly ranks areas equally well — the damage gradient
...
- **Two honesty guards baked into the figure:** (1) crowd COVERAGE varies — MS's 2.9× rests on
  ...
  red); (2) **no day-zero baseline overlay** — MapSwipe is seeded from AI flags, so it can't
