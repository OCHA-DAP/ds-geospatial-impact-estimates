...
# RQ3g — ranking by damage FRACTION instead of count: nothing flips (2026-09-02, external review)

Reviewer question (Leo): are we ranking damage rate or total damaged buildings — urban
vs rural could differ. The published ranking is COUNT per cell. Re-ranked by damage
fraction (damaged ÷ building stock; cells ≥20 buildings, rate noise below that), same
pipeline as rq3f (anchor: count columns reproduce rq3f_null_ranking.csv exactly).
Script `RQ3-prioritization-error-structure/scripts/rq3g_frac_ranking.py`:

```{python}
tbl("RQ3-prioritization-error-structure/rq3g_frac_vs_count.csv")
```

![](artefacts/RQ3-prioritization-error-structure/figs/rq3g_frac_vs_count_rho.png)

- **The "count flatters geography" hypothesis fails**: the null ranks fractions about
  as well as counts (ρ 0.43–0.65 both metrics). Damage *rate* is geographically
  structured (coastal gradient), so dividing out building stock removes nothing the
  null uses. Only OSU beats its null under either target, at either scale — the
  published conclusion is target-invariant.
- **Grade-bias split, fifth appearance**: the radar products gain under fractions
  (IMPACT 0.29→0.46, LIST 0.28→0.35 at res 8), appearance products lose (MS 0.47→0.37,
  UH −0.11→−0.21).
- Scatter grid (fractions, res 8): `figs/rq3g_frac_scatter_res8.png` — diagnostic only.
- Caveats: fraction cells restricted to ≥20 buildings; CEMS incompleteness biases
  measured rates down everywhere (ranks care only about the spatial pattern of the
  undercount). Scope 'all' (as-delivered); Caraballeda/core scopes not re-run.
- **Confidence: high** (frozen basis, exact anchor). Manuscript: paragraph + figure in
  #app-ranking-tables; half-sentence pointer in #sec-ranking.

# RQ3h — the agreement dial also RANKS: mid-k beats every single (2026-09-02, external review)

Reviewer objection (Leo): the closing recommendation ("count agreement") was proven at
building level only; ranking was never tested for the voting system. Tested on the
rq3f core frame (fixed shared cell set, res 9/8), with vote-sum, k-of-6 cell counts
and the fusion expected count added to the frozen singles/null. Anchors: singles and
null reproduce rq3f_null_ranking_core.csv exactly. Script
`RQ3-prioritization-error-structure/scripts/rq3h_agreement_ranking.py`; figure
`rq3h_agreement_ranking_fig.py` (manuscript #fig-agreerank in #sec-dial):

```{python}
tbl("RQ3-prioritization-error-structure/rq3h_agreement_ranking.csv")
```

![](artefacts/RQ3-prioritization-error-structure/figs/rq3h_agreement_ranking.png)

- **Mid-dial out-ranks everything**: 3/4-of-6 ρ 0.78 vs best single (LIST) 0.74 at
  res 8; 0.66 vs 0.56 at res 9. Beats fusion too (0.72/0.58) — fusion was trained to
  score buildings, not order cells, and its geography inputs smooth away inter-cell
  contrast.
- **Two metrics answer two questions and reward different k**: ρ (full ordering) peaks
  at 3–4-of-6; top-20 shortlist overlap peaks at 5-of-6 (0.78 tie-aware at res 8;
  res 9: 0.52 vs best single 0.20 and null 0.00).
- **Null top-20 dig ("right neighbourhood, wrong block")**: at res 9 the null ranks the
  true worst-20 cells at 26–271 of 594 — consistently top-fifth, never top-20. Its
  failure is resolution, not location; corroborated products resolve the block.
- **Tie-aware top-20** (`top20_exp`): at high k many cells tie at 1 flagged building
  and the naive top-20 depends on sort order; expected overlap credits each tied cell
  at slots/tie-group-size. Moves 5-of-6 0.80→0.78, 6-of-6 0.65→0.67; nothing else.
- **Open item (parked 2026-09-02)**: a near-miss ranking diagnostic (median rank the
  predictor assigns the true worst-20; or NDCG@20 if a reviewer demands the standard
  form) — the "ranks it gives the truth" view made the null dig legible and may be
  worth reporting alongside ρ/top-20. Not integrated yet.
- **No bootstrap intervals on these ρ yet** — margins like 0.78 vs 0.74 are reported
  without CIs; direction is consistent across both resolutions and both metrics.
- **Confidence: high** on direction (exact anchors, two resolutions, two metrics);
  medium on margins (no CIs). Manuscript: #fig-agreerank + paragraph in #sec-dial,
  pointer in #sec-ranking, recommendation sentence now carries the ranking evidence.
...
- **Two behaviours, roughly by sensing:** appearance/debris products **MS, UH, UNEP** are
  ...
  Radar **OSU** (coherence) **and IMPACT** (backscatter amplitude, ADR-0015; CORRECTED 2026-09-17, earlier text called both coherence) are grade-NEUTRAL on both (P and R ~equal for damaged vs
  destroyed) — radar change senses structural alteration, not visible collapse. **LIST sides with
  the radar group — and LIST is a SAR ResNet per the provider (ADR-0020)**, so its
...
- **Confidence: high** (683–784 points per grade). Manuscript: covered (2026-08-13) — one
  sentence in the floor section's Result plus the grade-mapping/exclusion rule in
  #app-matching; register canonical.
...
# RQ2p — OSU v1's confidence tiers: ordered, real, still under the ceiling (2026-08-12, user question)

User question: OSU's classes were never evaluated per tier — v0's continuous
`damage_probability` is too saturated to sweep (51% of flags at exactly 1.0, 62% ≥ 0.99),
but v1's ordinal tiers (probable / high_confidence) can be scored. Script
`RQ2-cems-footprint-points/scripts/rq2p_osu_v1_tiers.py`, three frames: v1's own extent
∩ CEMS, the RQ2h common extent, and the RQ5b core region (the core-region v0 row is the
reproduction anchor — it matches the frozen rq5b OSU row, P 0.045 / R 0.688):

```{python}
tbl("RQ2-cems-footprint-points/rq2p_osu_v1_tiers.csv")
```

- **The tiers are real and ordered**: core-region precision 0.059 (high_confidence) vs
  0.032 (probable), ~1.8×; the high-confidence cut is v1's best operating point (F1 0.103
  vs the published headline's 0.085 — half the flags, two-thirds of the recall).
- **Still under the ceiling**: the best tier sits below Microsoft's core precision (0.093)
  and nowhere near the voting rules (flat-voting best F1 0.290). RQ2d's "no hidden
  headroom, but a real quality signal" now generalizes beyond Microsoft — second product,
  same shape.
- v1 is post-freeze, so this enters the manuscript only via the version postscript
  (#app-osu-v1) plus a one-line cross-ref in #app-ceiling item 2; the paper basis stays v0.
- **Confidence: high** (id-keyed, frozen matching method, reproduction anchors pass);
  precision floors as always (incomplete reference).

# RQ2q — sensitivity: include CEMS 'possibly damaged'? Nothing qualitative changes (2026-08-19, user question)

The paper excludes class 1 from the reference; the only prior incl-possibly check
(rq2_points, 3 products) predates the freeze (pre-MONIT2 reference, pre-merged-refresh
MS), so its levels are dead. Re-run on the frozen basis — gold flags, CEMS latest, RQ5b
core region, r=10 centroid; the {2,3} side reproduces the frozen rq5b scorecard exactly.
Script `RQ2-cems-footprint-points/scripts/rq2q_incl_possibly.py`:

```{python}
tbl("RQ2-cems-footprint-points/rq2q_incl_possibly.csv")
```

- Reference grows 1,467 → 2,786 core points (+90%). **Every precision rises ~1.6–2×,
  mechanically** (more targets to be near) — singles and all six voting rules alike.
- **Recall moves along the RQ2l grade slope**: grade-neutral products gain or hold (OSU
  0.688→0.717, LIST 0.564→0.574, IMPACT ~flat) while the destruction-biased trio loses
  (MS 0.474→0.427, UH 0.337→0.260, UNEP 0.313→0.281) — class-1 points are exactly the
  damage the appearance products miss. A third independent appearance of the grade-bias
  split.
- **No conclusion changes**: voting still beats every single, the dial stays monotone in
  precision, 6-of-6 stays the precision extreme (0.640→0.729). Tiny precision reshuffles
  among the statistically tied top singles (UH/UNEP/MS swap places) sit inside the RQ9
  intervals.
- **Confidence: high** (frozen basis, exact reproduction anchor). Manuscript: one
  sentence in #app-matching's grade-rule paragraph. Exclusion is the more demanding
  choice for the products; the paper keeps it.

# RQ2r — precision lower → upper bound per product, one frame (2026-08-28, user request)

Composes the two reference corrections the paper had only quoted separately — CEMS
class 1 (RQ2q) and crowd-confirmed damage (RQ2i/RQ5b) — into one bounded interval per
product, all in the core region at r = 10 m, flags fixed. The upper bound is the UNION
{1,2,3} ∪ crowd-damaged, each flag credited once. Script
`RQ2-cems-footprint-points/scripts/rq2r_precision_bounds.py`; figure
`rq2r_bounds_fig.py` (manuscript #fig-bounds in #sec-floor):

```{python}
tbl("RQ2-cems-footprint-points/rq2r_precision_bounds.csv")
```

![](artefacts/RQ2-cems-footprint-points/figs/rq2r_bounds_ladder.png)

- **Lower → upper bound is 2.2–3.6×** (MS 0.081→0.279, OSU 0.045→0.160). The two
  corrections barely overlap: only 10–17% of crowd-confirmed CEMS-false flags sit within
  10 m of a class-1 point (`crowd_fp_near_class1`), so the crowd was mostly finding
  damage CEMS never mapped, not re-grading CEMS's own low-grade calls — the union is
  nearly the additive stack and does not double-count.
- **Convention audit (the reason this artefact carries two crowd columns).** The paper's
  artefacts held two different crowd-adjustment rules under one name: rq2i (and the
  #app-crowd re-vote deltas) credit only flags the crowd actually reviewed
  (*measured*), while rq5b (#tbl-dial's crowd-adj column) assumes unreviewed flags
  contain damage at the reviewed-flag rate (*extrapolated*). They agree at high crowd
  coverage (MS 98%: 0.239 vs 0.242) and diverge badly at low (UH 27%: 0.182 vs 0.418).
  The figure draws the **measured** convention (user decision, 2026-08-28); #tbl-dial's
  caption and #app-crowd now state which rule each number uses.
- **Anchors (all exact): P_floor/P_grade reproduce rq2q; P_crowd_extrap reproduces
  rq5b's P_crowd_adj** — proving the two published conventions reconcile through one
  computation.
- **Confidence: high** (frozen basis, three exact reproduction anchors). Caveat: the
  measured upper bound is itself understated where crowd coverage is thin (UH 27%),
  which the figure annotates per product.

# RQ8c — reference-basis sensitivity for the best-F1 figure's ten predictors (2026-08-28, user request)

Extends RQ2q from products+voting to ALL ten #fig-bestf1 predictors (six products, flat
voting, weighted fusion, both geography nulls) across three reference bases — {3},
{2,3}, {1,2,3} — with everything else frozen: RQ8 OOF scores, gold flags, operating
points recovered exactly from `n_flags` (no refits; refitting would let our
constructions adapt to each target while the shipped products cannot). rq8-style
building-denominator recall, NOT rq2q's point-denominator. Script
`RQ8-learned-fusion/scripts/rq8c_basis_sensitivity.py`; figures `rq8c_basis_fig.py`
(precision panel → manuscript #app-matching):

```{python}
tbl("RQ8-learned-fusion/rq8c_basis_pr_r10.csv")
```

![](artefacts/RQ8-learned-fusion/figs/rq8c_basis_prf1_r10.png)

- **Precision roughly doubles per step for everyone — mechanically** (same flags, more
  reference). The paper's two core comparisons are basis-invariant: fusion's precision
  stays ~3–4× the best single product at every basis (0.206/0.297/0.423), and the
  logistic geography null stays inside the product band at every basis.
- **The non-mechanical finding is in F1**: weighted fusion is nearly flat across bases
  (0.306/0.343/0.339) while flat k-of-6 voting falls monotonically (0.318/0.290/0.244)
  — the learned weighting is robust to the reference definition; the flat vote is not.
- **Recall splits along the RQ2l grade slope** (fourth independent appearance):
  grade-neutral products hold as lesser damage enters the target; destruction-biased
  products fall fastest.
- **Anchor: the {2,3} rows reproduce rq8_best_f1_r10.csv exactly** (all ten predictors;
  the rounded `threshold` column moves one building across the cut for the learned
  scores, so operating points are recovered from `n_flags`).
- **Confidence: high** (frozen scores, exact reproduction anchor).
...
## 2026-08-07 — Microsoft's public report, cross-read against our findings

Read their report (visualizers.aiforgood.ai/damage-assessment/venezuela_earthquake_2026_report.html)
after the v3 draft. Four takeaways:

- **Producer-side corroboration of the west-strip mechanism**: they name the Catia La
  Mar overlap as their single largest inter-scene disagreement — the Vantor 25 June
  scene flags ~2,600 buildings the Planet 26 June scene calls intact — resolved in
  overlap zones by preferring Planet. Our contested strip is exactly where no second
  scene existed to overrule the first. Added to manuscript v3's case-study section.
  Their 17.5% multi-scene disagreement rate also squares with RQ2d's corroboration
  signal.
- **Self-validated per-scene metrics** (e.g. Caraballeda recall 92% / precision 100%)
  come from 200–300 footprints per scene labeled by their own annotators on the same
  imagery; the report states no held-out status, no sampling method, no annotator
  independence, and no stratification by their own `unknown_pct` or confidence fields.
  Not a contradiction of our interval (0.08 floor → ~0.24 crowd-adjusted): theirs
  measures model-vs-visible-in-image, ours measures flags-vs-ground references. The
  in-event instance of the self-validation gap (cf. the Turkey citation).
- **Their cloud figure (4.2% obscured) is consistent with our RQ0 check (3.6%
  majority-obscured, precision unchanged under any visibility filter).**
- **New candidate reference**: 925 terremotovenezuela.com citizen reports (221 in their
  area), severity-graded, location error median 2.1 m with a long tail. Logged in
  OPEN-ITEMS as a possible fourth reference; check overlap with ChatMap first.

# RQ8d — is coast distance a fair null covariate? Feature ablation (2026-09-02, external review)

Objection (user, after Leo's null comments): the null's coast-distance feature looks
event-specific, "just luck". Ablated on the frozen core buildings, exact rq3f null
pipeline; the paper arm anchors to the rq3f core null exactly (rho .648/.478). Script
`RQ8-learned-fusion/scripts/rq8d_null_ablation.py` (DEM auto-fetched; soil arm behind
`--with-soil`):

```{python}
tbl("RQ8-learned-fusion/rq8d_null_ablation.csv")
```

- **The conclusions never depended on coast.** Density alone (nothing event-specific)
  scores F1 .086, inside the product band (.085–.148), ranking rho .543 at res 8.
  Coast adds ~+.04 F1 / +.10 rho over that.
- **A fully generalizable DEM null is STRONGER than the published one**: Copernicus
  GLO-30 elevation alone beats the paper's coast set (F1 .143 vs .127, rho8 .706 vs
  .648); with slope added, coast contributes almost nothing on top (+.007 F1). If
  anything the published null was undertuned — no product was flattered.
- **Physical reading**: within the 61 km² core no hazard covariate varies (one MMI
  band; the finite-fault surface projection covers the strip, Rjb ≈ 0 everywhere).
  Elevation works as a substrate covariate: the low-lying surfaces ARE the debris-flow
  fans USGS mapped after the 1999 Vargas disaster (Wieczorek et al. OFR 01-0144;
  I-2772) — named physics, globally available day-zero.
- **Two instructive dead ends, kept as rows**: (1) distance to the rupture surface
  projection degenerates (median 0 km) into an accidental beachfront detector with a
  physically backwards sign — F1 .226 is an artifact, do not cite; (2) SoilGrids
  sand/clay adds nothing (F1 .085): global soil products mask built-up land (86% of
  core buildings NoData, nearest-filled from ~310 m) and are near-constant across the
  strip. Local alternatives (FUNVISIS microzonation, INGEOMIN geology, the 1999 fan
  plates) exist only as PDF cartography.
- **Open item**: digitizing the Caraballeda fan outlines from OFR 01-0144 plates as a
  categorical substrate covariate — parked.
- **Confidence: high** (frozen basis, exact anchor). Manuscript: paragraph + table in
  #app-fair. Frozen v3 keeps its published null (all anchors intact); the brief/summary
  describes the null as "building density and terrain".
...
- **2026-08-07 — r30 refit for the unified radius table.** rq8 re-run at GIE_LABEL_R=30
  (new frozen `_r30` CSVs) so manuscript v3's appendix can show every predictor at
  10/20/30 m from one machinery (@tbl-radii there). Pattern: all numbers rise with
  radius; the null gains fastest (F1 0.128/0.206/0.282, matching or beating 3/4/5 of 6
  products) — the smeared-target mechanism confirmed at a third point; voting
  (0.290/0.342/0.364) and fusion (0.343/0.386/0.427) stay ahead at every radius.
  Companion run 2026-08-09: rq8b as-delivered baseline at r30
  (rq8b_asdelivered_baseline_r30.csv) so v3's radius table can pair every product cell
  as core (as-delivered) at all three radii; models keep core-only (the as-delivered
  benchmark is per-footprint by construction).
- **2026-08-10 — block-bootstrap CIs land (OPEN-ITEMS item 2).** New artefact
  RQ9-uncertainty: spatial block bootstrap, 2,000 reps, res-8 cells core / res-7
  as-delivered, one shared cell draw per lens (paired differences valid), match
  indicators computed once and asserted against frozen rq5b/rq2i/rq8 values; models
  scored from rq8's frozen OOF scores at frozen cuts (rq8_oof_scores_r10.parquet,
  GIE_DUMP_OOF=1). Headline: no product-minus-null F1 interval excludes zero upward
  (OSU's sits entirely BELOW, −0.076 to −0.012); flat voting +0.158 (0.081–0.227) and
  fusion +0.214 (0.162–0.256) separate decisively; 6-of-6 visits/find interval 0.56–0.96
  (sub-1 survives); as-delivered UH precision 0.009 (0.003–0.018). CSVs rq9_ci_core /
  rq9_ci_asdelivered_r10 / rq9_ci_models_r10; v3 gains #app-ci + pointers; "pending"
  sentence deleted. NOT covered: OSU v0-vs-v1 recall difference (rq2h frame) — noted in
  OPEN-ITEMS as an optional extension.
- **2026-08-10 — block-bootstrap CIs land (OPEN-ITEMS item 2).** New artefact
  RQ9-uncertainty: spatial block bootstrap, 2,000 reps, res-8 cells core / res-7
  as-delivered, one shared cell draw per lens (paired differences valid), match
  indicators computed once and asserted against frozen rq5b/rq2i/rq8 values; models
  ...
  sentence deleted. NOT covered: OSU v0-vs-v1 recall difference (rq2h frame) — noted in
  OPEN-ITEMS as an optional extension.
- **2026-08-10 — buffered-CV check: the leakage attack is dead.** rq8 gained
  GIE_CV_BUFFER_M (per fold, training buildings within the buffer of any test building
  are dropped; ablation refits too). At 300/500 m: null logistic F1 0.128→0.126/0.126,
  AP 0.060→0.059/0.058; fusion F1 0.343→0.345/0.340, AP 0.284→0.279/0.277. All
  movement <0.005 — the unbuffered design flattered nothing material. CSVs `_buf300`
  / `_buf500`; Limitations + appendix fairness section updated in manuscript v3.
