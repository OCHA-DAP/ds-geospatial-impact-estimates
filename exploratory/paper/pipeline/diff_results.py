...
REL = os.path.relpath(os.path.join(HERE, "..", "artefacts"), ROOT)   # old refs hold the per-RQ CSVs under artefacts/
...
def build_at(ref: str, tmp: str) -> pd.DataFrame:
    ...
    # The temp dir mimics the paper root: pipeline/consolidate.py reads ../artefacts/<rel> in --oracle
    # mode and writes ../results_oracle_frozen.csv, so lay the ref's CSVs out under tmp/artefacts/.
    os.makedirs(os.path.join(tmp, "pipeline"), exist_ok=True)
    shutil.copy(os.path.join(HERE, "consolidate.py"), os.path.join(tmp, "pipeline"))
    ...
    cons.LOADED.clear()
    cons.ORACLE_MODE = True   # discovery must read the archive, not frozen/: frozen/ holds only the heavy files
    ...
    for rel in sorted(inputs):
        os.makedirs(os.path.join(art, os.path.dirname(rel)), exist_ok=True)
        try:
            blob = subprocess.check_output(["git", "show", f"{ref}:{REL}/{rel}"], cwd=ROOT, stderr=subprocess.DEVNULL)
            open(os.path.join(art, rel), "wb").write(blob)
    ...
    if missing:
        ...
        for rel in missing:  # write an empty frame with the current header so the source function raises a clear error
            hdr = open(os.path.join(HERE, "..", "artefacts", rel)).readline()
            open(os.path.join(art, rel), "w").write(hdr)
    ...
    for rel in sorted(inputs):
        f = os.path.join(art, rel)
        df = pd.read_csv(f)
        ren = {k: v for k, v in COMPAT.items() if k in df.columns and v not in df.columns}
        if ren:
            df.rename(columns=ren).to_csv(f, index=False)
    r = subprocess.run([sys.executable, "consolidate.py", "--oracle"], cwd=os.path.join(tmp, "pipeline"), capture_output=True, text=True)
    if r.returncode:
...
def expectations(m: pd.DataFrame) -> list:
    ...
    # Id-keyed products (IMPACT, OSU, LIST) are untouched by any mapping rule; UNEP and UH are re-mapped by the
    # one-rule footprint mapping (ADR-0030 amendment) and may move by the amounts diff --expect footprint-rule verified.
    d = sel("core", "points", 10, "n_flags", ["IMPACT", "OSU", "LIST"]); out.append(("id-keyed products' core flag counts move by <= 3 (IMPACT, OSU, LIST)", bool(((d.value_now - d.value_base).abs() <= 3).all()), f"max |Δ| {(d.value_now - d.value_base).abs().max():g}"))
    d = sel("core", "points", 10, "n_flags", ["UNEP", "UH"]); out.append(("UNEP and UH core flag counts move by <= 2% (one-rule footprint mapping)", bool((((d.value_now - d.value_base).abs() / d.value_base) <= 0.02).all()), ", ".join(f"{p} {int(b)} -> {int(n)}" for p, b, n in zip(d.predictor, d.value_base, d.value_now))))
    ...
    unchanged, remapped = ["IMPACT v2", "OSU", "LIST"], ["Microsoft", "UNEP", "UH"]
    d = m[(m.lens == "cells") & (m.metric == "rho") & (m._merge == "both") & m.predictor.isin(unchanged)]; out.append(("area-ranking correlations move by <= 0.011 for products whose flags did not change (IMPACT, OSU, LIST)", bool(((d.value_now - d.value_base).abs() <= 0.011).all()), f"max |Δ| {(d.value_now - d.value_base).abs().max():.3f} over {len(d)} rows"))
    d = m[(m.lens == "cells") & (m.metric == "rho") & (m._merge == "both") & m.predictor.isin(remapped)]; out.append(("area-ranking correlations move by <= 0.02 for the re-mapped products (Microsoft, UNEP, UH)", bool(((d.value_now - d.value_base).abs() <= 0.02).all()), f"max |Δ| {(d.value_now - d.value_base).abs().max():.3f} over {len(d)} rows"))
