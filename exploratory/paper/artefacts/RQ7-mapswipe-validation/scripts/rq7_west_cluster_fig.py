...
Run: uv run --with pandas --with scipy --with matplotlib --with h3 python \
...
def main() -> None:
    ...
    s0 = ax_res.scatter(ll[:, 1], ll[:, 0], c=j.resid, cmap="RdBu_r",
                        vmin=-lim, vmax=lim, s=70)
    ...
    s1 = ax_crowd.scatter(ll[:, 1], ll[:, 0], c=j.rej_share, cmap="PuOr_r",
                          vmin=0, vmax=1, s=70)
