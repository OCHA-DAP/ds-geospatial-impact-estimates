...
ax.scatter([], [], c="#4a5560", s=70, label="single products (dot = fair cost)")
...
ax.scatter(kx, [cost(p) for p in kof.P_crowd_adj], c="#2a78d6", s=200, marker="s",
           zorder=5, label="k-of-6 agreement (square = fair cost,\nbar top = worst-case cost)")
for _, r in kof.iterrows():
...
ax.set_xlabel("coverage — share of field-verified damage the list reaches (r = 20 m)",
              fontsize=12)
ax.set_ylabel("cost — field visits per verified damaged building found\n"
              "(log scale; lower = better)", fontsize=11)
ax.set_yticks([1, 2, 3, 5, 8, 15, 30, 60])
ax.set_yticklabels([1, 2, 3, 5, 8, 15, 30, 60])
ax.tick_params(labelsize=11)
ax.legend(fontsize=10, loc="upper center")
ax.set_title("Is combining products 'good enough'? The honest operational trade.\n"
             "Best case (Caraballeda). Each bar = cost range (crowd-adjusted → CEMS-only); "
