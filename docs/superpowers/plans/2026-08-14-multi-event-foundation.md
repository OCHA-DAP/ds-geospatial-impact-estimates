...
- Trunk is `v1` (NOT `main` — `main` is empty). **Never commit to `v1` directly** — all work goes on feature branches merged via PR. `swa-deploy.yml` deploys SWA **prod** on every push to `v1` touching `web/**`, and PRs touching `web/**` get an SWA preview env on staging data. Branch topology: Task 1 on `sever-appservice-fallback` (own PR, merged early); Tasks 2–6, 9, 10 on `multi-event-foundation` (one PR); Tasks 7–8 on `multi-event-spa` cut from current `v1` (own PR, merged in Task 11 — the SPA depends only on published blob data, not on the foundation branch's Python).
